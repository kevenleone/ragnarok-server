...
class MonsterDAO extends Model {
    constructor() {
        super();
    }

    getMonster(start) {
        return this.query(`select id, iName, r.race from mob_db mb join race r on r.id_race = mb.Race group by iName limit ${start}, 300`)
    }

    getMonsterDrops(MonsterId) {
        let query = `select 
    ...
    getMonsterDetail(idMonster) {
        return this.query(`select * from mob_db mb join race r on r.id_race = mb.Race where mb.id =  ${idMonster}`)
    }

    getMonsterSkill(idMonster) {
       return this.query(`select sd.SkillID, CONCAT(sd.Description, ' (',SKILL_LV,')') as skill from mob_skill_db msd 
    ...
    getMonsterHome(idMonster) {
        return this.query(`select map, spawntime, sum(quantity) total from mob_place 
        where mobId = ${idMonster} group by map order by map`);
    }
}
